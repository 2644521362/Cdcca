...
class MetaModel(nn.Module):
    ...
    def generate(
        ...
        for cur_pos in range(start_pos, total_len):
            if flag == False:
                logits, image_tokens = self.llma.forward_inference(tokens[:, prev_pos:cur_pos], prev_pos, images if prev_pos == 0 else None)
                flag = True
            else:
                logits, _ = self.llma.forward_inference(tokens[:, prev_pos:cur_pos], prev_pos, images if prev_pos == 0 else None)
            if temperature > 0:
                probs = torch.softmax(logits / temperature, dim=-1)
                probs_list.append(torch.max(probs))
        ...
        return decoded, probs_list, image_tokens
