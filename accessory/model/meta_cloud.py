...
class MetaModel(nn.Module):
    ...
    def __init__(
        ...
        model_args: ModelArgs = ModelArgs(
            max_seq_len=max_seq_len, max_batch_size=1, **params
        )
    ...
    def forward(self, examples, labels, images=None):
        ...
        output, KD_loss, KD_feature_loss = self.llma(examples, images)
        output = output[:, :-1, :]
        labels = labels[:, 1:]
        ...
        return c_loss, KD_loss, KD_feature_loss


    def generate(
        self,
        prompts: List[str],
        uts_tokens: torch.Tensor,
        max_gen_len: int,
        ...
        for cur_pos in range(start_pos, total_len):
            logits = self.llma.forward_inference(tokens[:, prev_pos:cur_pos], prev_pos, uts_tokens if prev_pos == 0 else None)
            if temperature > 0:
