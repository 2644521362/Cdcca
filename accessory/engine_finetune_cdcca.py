...
def train_one_epoch(model: torch.nn.Module,
    ...
    for data_iter_step, batch_data in enumerate(
        ...
        with autocast_ctx:
             c_loss, KD_loss, KD_feature_loss = model(examples, labels, images=imgs)
        loss = c_loss + KD_loss + KD_feature_loss
        loss_value = loss.item()
        c_loss_value = c_loss.item()
        KD_loss_value = KD_loss.item()
        KD_feature_loss_value = KD_feature_loss.item()
        ...
        KD_loss_value_reduce = misc.all_reduce_mean(KD_loss_value)
        KD_feature_loss_value_reduce = misc.all_reduce_mean(KD_feature_loss_value)
